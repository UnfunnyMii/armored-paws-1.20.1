...
public class ArmadilloBrain {
    ...
    private static final double field_48338 = (double)2.0F;
    private static final double field_48339 = (double)1.0F;
    ...
    private static void addIdleActivities(Brain<ArmadilloEntity> brain) {
        brain.setTaskList(Activity.IDLE, ImmutableList.of(Pair.of(0, LookAtMobWithIntervalTask.follow(EntityType.PLAYER, 6.0F, UniformIntProvider.create(30, 60))), Pair.of(1, new BreedTask(ModEntities.ARMADILLO, 1.0F)), Pair.of(2, new RandomTask(ImmutableList.of(Pair.of(new TemptTask((armadillo) -> 1.25F, (armadillo) -> armadillo.isBaby() ? (double)1.0F : (double)2.0F), 1), Pair.of(WalkTowardClosestAdultTask.create(WALK_TOWARDS_CLOSEST_ADULT_RANGE, 1.25F), 1)))), Pair.of(3, new RandomLookAroundTask(UniformIntProvider.create(150, 250), 30.0F, 0.0F, 0.0F)), Pair.of(4, new RandomTask(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryModuleState.VALUE_ABSENT), ImmutableList.of(Pair.of(StrollTask.create(1.0F), 1), Pair.of(GoTowardsLookTargetTask.create(1.0F, 3), 1), Pair.of(new WaitTask(30, 60), 1))))));
    }
    ...
    static {
        ...
        UNROLL_TASK = TaskTriggerer.task((context) -> context.group(context.queryMemoryAbsent(MemoryModuleType.DANGER_DETECTED_RECENTLY)).apply(context, (memoryQueryResult) -> (serverWorld, armadillo, l) -> {
